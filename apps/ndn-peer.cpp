...
// Update weights
void
Peer::UpdateWeights(Data tail, std::vector<Name> visited) {
  ...
  for (size_t i = 0; i != approvedBlocks.size(); i++) {
    ...
    if (std::find(processed.begin(), processed.end(), approvedBlock) == processed.end()) {
      ...
      if (std::find(visited.begin(), visited.end(), approvedBlock) == visited.end()) {
        auto it = m_weightList.find(approvedBlock);
        if (it != m_weightList.end()) { // this should always return true
          m_weightList.insert(std::pair<Name, int>(approvedBlock, it->second + 1));
        }
        processed.push_back(approvedBlock);
        UpdateWeights(m_ledger.find(approvedBlock)->second, visited);
      }
    }
  }
}
